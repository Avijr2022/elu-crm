...
| 1.29 | 2026-09-23 | EIIP / Engineering (governance reconciliation; approver identity not supplied in the authorising instruction; ELU-AI-001) | PF-008 Users & Identity release recorded from verified git facts: implementation commit 771f846 merged via PR #28 at release baseline 20320125c4f37094b41e21f5800934699c881a66, annotated tag `Phase-2-PF008` (tag object 2b5ab64831912de65fb47c601c40cce0018715d1). Status corrected from NOT STARTED to RELEASED and the progress dashboard arithmetic corrected to 8 / 11 (73%). **Recording and verification only; no implementation, release approval or tag action is implied.** |
| 1.30 | 2026-09-23 | EIIP / Engineering (governance recording; approver identity not supplied in the authorising instruction; ELU-AI-001) | **PF-009 BATCH 2 AUTHORIZED: YES** — human authorization recorded verbatim for the PF-009 Batch 2 scope: (1) runtime permission-grain enforcement in `app.core.rbac.has_permission`; (2) removal of the PLATFORM_ADMIN universal bypass so the seeded permission matrix governs; (3) reconciliation of the PF-005/PF-006/PF-007 gates that currently bypass permission-grain checks; (4) addition/adjustment of tests covering the resulting runtime permission behaviour. **Recording only; no implementation, no release approval, no commit and no tag is implied.** |
| 1.31 | 2026-09-23 | EIIP / Engineering (governance reconciliation; approver identity not supplied in the authorising instruction; ELU-AI-001) | **PF-009 Batch 2 runtime permission grain — IMPLEMENTED, COMMITTED and PUSHED to `master` — NOT released.** Implementation commit `d4e9659f7ccb7c2bc877058be63e4e10db01cb42` (9 files, +334 / -101; parent `a0a861d45ba026e88ff6e23ab34fc4535aef18b6`), pushed to `origin/master` (`a0a861d..d4e9659`, fast-forward) — `master` = `origin/master` = `d4e9659`; total tags **12**, none created, moved or deleted. **Implemented:** (1) `app.core.rbac.has_permission` decides on the seeded permission grain; (2) the PLATFORM_ADMIN universal bypass is removed so the seeded matrix governs; (3) the PF-005/PF-006/PF-007 service gates and their API call sites enforce `branch.*` / `department.*` / `business_unit.*` grains, with the create-route guards moved inside each router's `AppError → http_error_from_app` wrapper so guard denials return the standard error envelope; (4) `_sync_pf_module_matrices` is edition-aware — the `branch.*` / `business_unit.*` matrices are synced only when the tenant's edition carries `BRANCH` / `BUSINESS_UNIT`; (5) `Backend/tests/test_pf009_rbac_foundation.py` extended to **26 tests** (6 new). **Verified:** focused PF-005/006/007/008/009 **130 passed, exit 0**; `git diff --check` clean; `compileall -q Backend/app Backend/tests` exit 0; CI **CRM Tests** run `35886328014` **SUCCESS** (full backend suite + both Flutter projects). **Registered as pre-existing / out-of-scope (NOT fixed):** PF-004 fixture isolation — `tests/test_pf002_tenants.py` creates tenants whose admin carries the seed admin email while ~24 fixtures resolve the seed admin with an unfiltered lookup, which can mis-resolve on a reused local database; it does not reproduce in CI. **Not done:** no release approval, no certification, no tag, no PR, no merge, no `ELU-QA-PF009` release audit, the existing `ELU-REL-PF009-Phase-2-PF009-Release-Notes.md` covers **Batch 1** only — **no Batch 2 release notes** exist. |
| 1.32 | 2026-09-26 | EIIP / Engineering (governance recording; approver identity not supplied in the authorising instruction; ELU-AI-001) | **PF-010 CORE IMPLEMENTATION AUTHORIZED: YES** — scope `PF-010-001 Audit Trail` / `PF-010-001-001 Activity Logging`; decision: extend the existing `audit.audit_event` and `write_audit_event()` with no duplicate audit sink; acceptance AC-PF-010-01..06; retention 90 days / 1 year / 7 years; NTF-PF-010-01..04; RPT-PF-010-01..05; NTF-PF-010-02 and -04 deferred (spec-marked v2). Recorded as authorization only: no implementation, no commit, no push, no release approval and no tag. Work baseline `29f49f86cbb4a7bc7cfbe7cfbc24e776d418eb9e`. Recorded in `ELU-MSL-002` change log **4.98** and `Documentation/CHANGELOG.md` (2026-09-26). Tracker status row and dashboard deliberately left unchanged (implementation not started). |
...
Overall
Modules Completed (PF)     : 8 / 11  (73%)
Modules Remaining (PF)     : 3
